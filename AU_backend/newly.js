...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    await pool.query(insertPreferenceQuery, insertPreferenceValues);

    // Insert user's food_restrictions into the 'user_food_restrictions' table
    if (food_restrictions && food_restrictions.length > 0) {
      // Check if 'None' is present in food_restrictions
      if (food_restrictions.includes("None")) {
        // When 'None' is present, fetch the restriction_id for 'None'
        const fetchNoneRestrictionIdQuery =
          "SELECT restriction_id FROM food_restrictions WHERE restriction_name = $1";
        const fetchNoneRestrictionIdValues = ["None"];
        const noneRestrictionResult = await pool.query(
          fetchNoneRestrictionIdQuery,
          fetchNoneRestrictionIdValues
        );

        if (noneRestrictionResult.rows.length > 0) {
          const noneRestrictionId =
            noneRestrictionResult.rows[0].restriction_id;
          // Insert the 'None' restriction_id into the 'user_food_restrictions' table
          const insertRestrictionsQuery =
            "INSERT INTO user_food_restrictions (user_id, restriction_id) VALUES ($1, $2)";
          const insertRestrictionsValues = [user_id, noneRestrictionId];
          await pool.query(insertRestrictionsQuery, insertRestrictionsValues);
        } else {
          // 'None' restriction_id not found in the food_restrictions table, return an error
          return res.status(500).json({
            error: "The 'None' food restriction could not be found.",
          });
        }
      } else {
        // Fetch the restriction IDs from the 'food_restrictions' table for other restrictions
        const fetchRestrictionIdsQuery =
          "SELECT restriction_id FROM food_restrictions WHERE restriction_name = ANY($1)";
        const fetchRestrictionIdsValues = [foodRestrictionsToSend];

        const restrictionIdsResult = await pool.query(
          fetchRestrictionIdsQuery,
          fetchRestrictionIdsValues
        );
        ...
        // Use a loop to insert each restriction separately
        for (const restriction_id of restriction_ids) {
          const insertRestrictionsValues = [user_id, restriction_id];
          await pool.query(insertRestrictionsQuery, insertRestrictionsValues);
        }
      }
            ...
            pool.query(
              ...
              (err, mealsResult) => {
                ...
                const meals = mealsResult.rows.map((meal) => ({
                  ...
                  food_restrictions: meal.food_restrictions,
                  calories: meal.calories,
                  //photo_filepath: meal.photo_filepath, // Include the photo_filepath in the response
                  //photo_filepath: `/images/${meal.photo_filepath}`, // Return a relative URL
                  photo_filepath: meal.photo_filepath
                    ? `/images/${path.basename(meal.photo_filepath)}`
                    : null,
                }));

                // Logging the image URLs and checking if the files exist
                meals.forEach((meal) => {
                  if (meal.photo_filepath) {
                    const imagePath = path.join(
                      __dirname,
                      "images",
                      path.basename(meal.photo_filepath)
                    );
                    fs.pathExists(imagePath)
                      .then((exists) => {
                        if (exists) {
                          console.log(
                            "Image exists for meal_id",
                            meal.meal_id,
                            ":",
                            meal.photo_filepath
                          );
                ...
                if (
                  ...
                  pool.query(fetchAllMealsQuery, (err, allMealsResult) => {
                    ...
                    const allMeals = allMealsResult.rows.map((meal) => ({
                      ...
                      calories: meal.calories,
                      //photo_filepath: meal.photo_filepath, // Include the photo_filepath in the response
                      //photo_filepath: `/images/${meal.photo_filepath}`, // Return a relative URL for the photo_filepath
                      photo_filepath: meal.photo_filepath
                        ? `/images/${path.basename(meal.photo_filepath)}`
                        : null,
                    }));
...
/*app.get("/weekly-meals/:email/:password", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      // If the user has selected "None" for both diet preference and food restrictions,
      // fetch all meals available in the 'meals' table
      fetchMealsQuery = "SELECT meal_id, meal_name, photo_filepath FROM meals";
      fetchMealsValues = [];
    } else if (dietPreference === "None") {
      // If the user has selected "None" for diet preference but has specific food restrictions,
      // fetch meals that do not have any of the food restrictions
      fetchMealsQuery =
        "SELECT meal_id, meal_name, photo_filepath FROM meals WHERE NOT EXISTS (SELECT 1 FROM unnest($1::text[]) r(food) WHERE meals.food_restrictions @> ARRAY[r.food]) OR (food_restrictions IS NULL OR 'None' = ANY(food_restrictions))";
      fetchMealsValues = [foodRestrictions];
    } else if (foodRestrictions && foodRestrictions.includes("None")) {
      // If "None" is included in foodRestrictions, fetch all meals with the specified diet_preference
      fetchMealsQuery =
        "SELECT meal_id, meal_name, photo_filepath FROM meals WHERE diet_preference = $1";
      fetchMealsValues = [dietPreference];
    } else {
      // If the user has selected specific diet preference and specific food restrictions,
      // fetch meals that match the diet preference and do not have any of the food restrictions
      fetchMealsQuery =
        "SELECT meal_id, meal_name, photo_filepath FROM meals WHERE diet_preference = $1 AND NOT EXISTS (SELECT 1 FROM unnest($2::text[]) r(food) WHERE meals.food_restrictions @> ARRAY[r.food])";
      fetchMealsValues = [dietPreference, foodRestrictions];
    }*/
...
/*let fetchMealsQuery;
    ...
    if (
      ...
      // fetch meals that do not have any of the food restrictions
      fetchMealsQuery =
        "SELECT meal_id, meal_name, photo_filepath FROM meals WHERE NOT (food_restrictions @> ARRAY[$1]) OR (food_restrictions IS NULL OR 'None' = ANY(food_restrictions))";
      fetchMealsValues = [foodRestrictions];
    } else if (foodRestrictions && foodRestrictions.includes("None")) {
      // If "None" is included in foodRestrictions, fetch all meals with the specified diet_preference
      fetchMealsQuery =
      ...
      // fetch meals that match the diet preference and do not have any of the food restrictions
      fetchMealsQuery =
        "SELECT meal_id, meal_name, photo_filepath FROM meals WHERE diet_preference = $1 AND NOT (food_restrictions && ARRAY[$2])";
      fetchMealsValues = [dietPreference, foodRestrictions];
    }
...
/*app.get("/weekly-meals/:email/:password", async (req, res) => {
  ...
  try {
    ...
    // Now, let's insert the fetched weekly meals into the user_weekly_meals table
    try {
      // Begin the transaction
      await pool.query("BEGIN");

      // Delete existing weekly meals for this user from the user_weekly_meals table
      await pool.query("DELETE FROM user_weekly_meals WHERE user_id = $1", [
        userResult.rows[0].id,
      ]);

      // Generate and insert new weekly meals for this user into the user_weekly_meals table
      for (const meal of weeklyMeals) {
        const insertQuery =
          "INSERT INTO user_weekly_meals (user_id, day_of_week, meal_id) VALUES ($1, $2, $3)";
        const insertValues = [
          userResult.rows[0].id,
          meal.day_of_week,
          meal.meal_id,
      ...
      console.error("Error inserting weekly meals:", error);
      return res.status(500).json({ error: "Error inserting weekly meals" });
    }
  } catch (error) {
    console.error("Error fetching weekly meals:", error);
    return res.status(500).json({ error: "Error fetching weekly meals" });
