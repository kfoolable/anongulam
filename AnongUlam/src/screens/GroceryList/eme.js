...
const GroceryList = ({route, navigation}) => {
  ...
  useEffect(() => {
    // Fetch grocery list data from the backend using the user's email and password
  ...
  const handleAddToPantry = () => {
    ...
    // Get the unique ingredient IDs from the selected grocery items
    const ingredientIds = selectedGroceryItems.map(item => item.ingredient_id);
    ...
    console.log('Ingredient IDs to be removed:', ingredientIds); // Add this log
    console.log('Ingredient Names to be removed:', ingredientNames); // Add this log
...
const styles = StyleSheet.create({
  ...
  groceryListContainer: {
    height: 480, // Set the fixed height for the grocery list ScrollView
    borderWidth: 1,
    padding: 10,
    borderRadius: 5,
  },
  ...
  listContainer: {
    marginTop: 10,
    alignItems: 'center', // Center the list items and the button horizontally
  },
  groceryListItem: {
    flexDirection: 'row',
    alignItems: 'center',
